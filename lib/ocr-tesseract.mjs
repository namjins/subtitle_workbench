...
/**
 * Rules fitted to the the reference discs corpus: they name specific words
...
function isUsableFallback(candidate, profile = "generic") {
  if (!candidate.text) return false;
  // Literal strings from the the reference discs corpus, held to the same standard
  // as the fitted text rules rather than applying unconditionally.
  if (profile === "fitted" && (candidate.text === "H..." || candidate.text === "E...")) {
    return true;
  }
  ...
  if (/^[A-Z]\.\.\.$/u.test(candidate.text) || /^[A-Za-z]{1,2}$/u.test(candidate.text)) {
    return false;
  }
...
function isThresholdDilateFallback(candidate, profile) {
  if (!isUsableFallback(candidate, profile)) return false;
...
function shouldPreferThresholdDilate(primary, fallback, profile) {
  if (!isThresholdDilateFallback(fallback, profile)) return false;
  ...
  return {
    ...
    async recognize(imagePath, { language = "eng" } = {}) {
      ...
      if (alwaysTryThreshold) {
        ...
        if (isThresholdDilateFallback(thresholdCandidate, profile)) {
      ...
      if (
        ...
        alwaysTryThreshold &&
        thresholdCandidate &&
        shouldPreferThresholdDilate(selected, thresholdCandidate, profile)
      ) {
      ...
      if (!selected.text) {
        ...
        const fallbackCandidates = (await Promise.all([
          ...
          candidate.variant === "psm-10"
            ...
            : candidate.variant === "threshold-dilate-psm-6"
              ? isThresholdDilateFallback(candidate, profile)
            : isUsableFallback(candidate, profile),
        );
        ...
        candidates.push(thresholdCandidate);
        if (shouldPreferThresholdDilate(selected, thresholdCandidate, profile)) {
          selected = thresholdCandidate;
        }
      }
...
export function createOcrEngine(name = "auto", options = {}) {
  ...
  if (name === "auto") {
    return options.mode === "subidx-to-srt" && isMacosVisionAvailable()
      ? createMacosVisionEngine({ textCleanup })
      : createTesseractHybridEngine({ alwaysTryThreshold: true, textCleanup });
  }
