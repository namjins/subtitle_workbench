...
import { hasCommand, imageMagickCommand } from "./platform-paths.mjs";
...
async function transformImage(imagePath, suffix, args) {
  ...
  await run(imageMagickCommand() ?? "magick", [imagePath, ...args, `PNG24:${outputPath}`]);
  return outputPath;
}
...
async function grayHistogram(imagePath) {
  const histogram = await run(imageMagickCommand() ?? "magick", [
    imagePath,
    "-colorspace",
    "Gray",
    "-format",
    ...
    async recognize(imagePath, { language = "eng" } = {}) {
      ...
      if (alwaysTryThreshold) {
        ...
        [thresholdCandidate, ...repairCandidates] = await Promise.all([
          ...
          .../** @type {[string, (imagePath: string, bins: object) => Promise<string|null>][]} */ ([
