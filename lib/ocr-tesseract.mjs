...
function parseTesseractTsv(tsv, variant) {
...
function isUsableFallback(candidate) {
  if (!candidate.text) return false;
  if (candidate.text === "H..." || candidate.text === "E...") return true;
...
async function ocrCandidate(imagePath, language, psm, variant = `psm-${psm}`) {
  ...
  return {
    ...
    async recognize(imagePath, { language = "eng" } = {}) {
      ...
      const candidates = await Promise.all(
        ["6", "11"].map((psm) => ocrCandidate(imagePath, language, psm)),
      );
      ...
      if (!selected.text) {
        ...
        const fallbackCandidates = (await Promise.all([
          ocrCandidate(imagePath, language, "10"),
          thresholdCandidate ??
            ocrCandidate(thresholdPath, language, "6", "threshold-dilate-psm-6"),
          ocrCandidate(resize2Path, language, "7", "resize2-psm-7"),
          ocrCandidate(resize2Path, language, "13", "resize2-psm-13"),
          ocrCandidate(grayAutoPath, language, "6", "gray-auto-psm-6"),
          ocrCandidate(grayAutoPath, language, "13", "gray-auto-psm-13"),
          ocrCandidate(imagePath, language, "13"),
        ])).filter((candidate) =>
...
export function createOcrEngine(name = "auto", options = {}) {
  if (name === "auto") {
    return options.mode === "subidx-to-srt" && isMacosVisionAvailable()
      ? createMacosVisionEngine()
      : createTesseractHybridEngine({ alwaysTryThreshold: true });
  }
  if (name === "tesseract-accurate") {
    return createTesseractHybridEngine({ alwaysTryThreshold: true });
  }
  if (name === "tesseract" || name === "tesseract-hybrid") {
    return createTesseractHybridEngine();
  }
  if (name === "macos-vision") {
    ...
    return createMacosVisionEngine();
  }
