 ...
 */
async function shadowStripPath(imagePath) {
  ...
  return {
    ...
    async recognize(imagePath, { language = "eng" } = {}) {
      ...
      if (alwaysTryThreshold) {
        ...
        [thresholdCandidate, shadowCandidate] = await Promise.all([
          thresholdDilatePath(imagePath).then((path) =>
            ocrCandidate(path, language, "6", "threshold-dilate-psm-6", profile),
          ),
          shadowStripPath(imagePath).then((path) =>
            path
              ? ocrCandidate(path, language, "6", "shadow-strip-psm-6", profile)
              : null,
          ),
        ]);
        ...
        // Competes on score like every candidate: on a shadowed track the
        // stripped read comes back at clean-track confidence while the direct
        // reads sit 15+ points lower, so it wins exactly where it should.
        if (shadowCandidate && isUsableFallback(shadowCandidate)) {
