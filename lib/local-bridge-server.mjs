...
import { appVersion } from "./conversion-cache.mjs";
...
let activeJobs = 0;
// In-flight jobs by id, so /jobs/cancel can abort one explicitly. The id is
// an opaque server-minted UUID used only as a map key — network input never
// touches a path or a binary through this.
 ...
 * @param {Function} [options.runJob]
 * @param {Function} [options.pickFile]
 * @param {Function} [options.revealFile]
 * @param {string|null} [options.uiRoot]
...
export function createLocalBridgeServer(options = {}) {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === "POST" && request.url === "/jobs/cancel") {
        const body = await readJsonBody(request);
        const jobId = typeof body?.jobId === "string" ? body.jobId : "";
        const controller = cancellableJobs.get(jobId);
        controller?.abort();
        // Cancelling an already-finished (or unknown) job is a no-op, not an
        // error: the client learned the id from a stream that may have ended.
        sendJson(response, 200, { cancelled: Boolean(controller) });
        return;
      }
      ...
      // The id exists so a client can cancel *explicitly*. Connection close
      // (below) also cancels, but it is not sufficient on its own: WebKit can
      // keep an aborted fetch's connection open to drain it for reuse, so the
      // server never sees a close and the whole OCR tree ran on — found
      // exactly that way, four stopped runs still saturating the CPU.
      ...
      // Closing the tab must still stop the work: a crashed or closed client
      // never gets to send an explicit cancel.
      ...
      if (response.headersSent) {
        ...
        sendJson(
          response,
          /** @type {HttpError} */ (error).statusCode ?? 500,
          { error: error.message },
        );
      }
    }
  ...
  // Attached for callers (tests, the CLI banner); not part of http.Server.
  /** @type {import("node:http").Server & {sessionToken?: string|null}} */ (
    server
  ).sessionToken = token;
  return server;
}
 ...
 */
/**
 * @param {import("node:http").IncomingMessage | {headers?: Record<string, string>}} request
 * @param {{token?: string|null, devOrigins?: string[]}} [options]
 */
...
async function serveUiAsset(request, response, uiRoot, token) {
  ...
  if (token && extname(servedPath) === ".html") {
    ...
    // same-origin document can read it back out. The package version rides
    // along: the desktop shell loads whatever UI the *installed npm package*
    // serves, so without this a bug report cannot name the version that
    // actually ran.
    body = Buffer.from(
      body
        .toString("utf8")
        .replace(
          "</head>",
          `<script>window.__SUBTITLE_WORKBENCH_TOKEN__=${JSON.stringify(token)};` +
            `window.__SUBTITLE_WORKBENCH_VERSION__=${JSON.stringify(appVersion())}</script></head>`,
        ),
      "utf8",
    );
  }
...
async function pickLinuxFile({ extensions, multiple }) {
  ...
  try {
    ...
    if (error?.code === "ENOENT") {
      const unavailable = /** @type {HttpError} */ (
        new Error("Native file picking needs `zenity` installed on this system.")
      );
...
async function receiveUploadedFiles(request) {
  ...
  const webRequest = new Request("http://127.0.0.1/uploads", {
    ...
    // Node's fetch requires duplex for a streaming body; lib.dom's
    // RequestInit predates it.
    .../** @type {object} */ ({ duplex: "half" }),
  });
