 ...
 */
export function parseArgv(argv, { valueOptions = new Set(), hasCommand = true } = {}) {
  // Tools spawned without a subcommand (benchmark_ocr, extract_missing_sup_images)
  // receive their first flag at argv[2]. Treating it as the command silently
  // swallowed one option per invocation, which is why `hasCommand: false` exists.
  const command = hasCommand ? argv[2] : undefined;
  const positionals = [];
  const values = new Map();
  const booleans = new Set();
  let terminated = false;

  for (let index = hasCommand ? 3 : 2; index < argv.length; index += 1) {
    ...
    // Only these two single-dash literals are flags. Generalising to any `-x`
    // would make a negative option value like `--tolerance -0.5` unparseable.
    if (arg === "-h" || arg === "-v") {
      booleans.add(arg);
      continue;
    }
