...
export async function countPgsDisplaySets(inputPath) {
  ...
  while (offset + 13 <= data.length) {
    ...
    // Same truncation rule as scanDisplaySets: a segment whose payload runs
    // past the end of the file is not real. Without this the two walkers
    // disagreed about where a truncated .sup ends, and the empty-output guard
    // compares their answers.
    if (offset + 13 + length > data.length) break;
...
export async function extractPgsPreviewImages(inputPath, outputDirectory, count = 3) {
  ...
  scanDisplaySets(data, (image, pts) => {
    ...
    if (isUsableImage(image)) {
      ...
      // Encode immediately and retain only the compressed bytes. Keeping the
      // raw RGB for the whole scan held ~344 MB for a 24 MB source — the
      // conversion path calls this with count = Infinity, so a full feature
      // approached the heap limit before OCR had started. PNG is roughly an
      // order of magnitude smaller; width/height are captured here because the
      // raw image is gone by the write loop.
      pending.push({ png: encodePng(image), pts, width: image.width, height: image.height });
  ...
  for (const [index, item] of pending.entries()) {
    ...
    await writeFile(outputPath, item.png);
    outputs.push({
      path: outputPath,
      pts: item.pts,
      endPts: item.endPts,
