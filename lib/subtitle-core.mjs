...
export function parseFps(input = 23.976) {
  if (typeof input === "number") return Number.isFinite(input) && input > 0 ? input : 23.976;
  const value = String(input).trim();
  const fraction = value.match(/^(\d+(?:\.\d+)?)\s*\/\s*(\d+(?:\.\d+)?)$/u);
  if (fraction) {
    const numerator = Number(fraction[1]);
    const denominator = Number(fraction[2]);
    return denominator > 0 ? numerator / denominator : 23.976;
  }
...
function convertItt(source, fps) {
  const cues = [];
  ...
  for (const match of normalized.matchAll(paragraphPattern)) {
    ...
    const end =
      attrs.end ??
      (attrs.dur && begin
        ? String(parseTtmlTime(begin, fps) + parseTtmlTime(attrs.dur, fps))
        : undefined);
    const text = cleanSubtitleText(match[2]);
    if (!begin || !end || !text) continue;
    const start = parseTtmlTime(begin, fps);
    const stop = /^\d+(?:\.\d+)?$/u.test(end) ? Number(end) : parseTtmlTime(end, fps);
