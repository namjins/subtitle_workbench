...
/**
 * @param {object} [job]
 * @param {string} [job.command] required at runtime; the guard below throws without it
 * @param {string[]} [job.inputs]
 * @param {string} [job.language]
 * @param {string} [job.outDir]
 * @param {string|number} [job.jobs]
 * @param {string} [job.ocrEngine]
 * @param {string} [job.ocrCommand]
 * @param {boolean} [job.noCache]
 */
...
export function runLocalCommand(command, args, options = {}) {
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    child.on("close", (status, signal) => {
      if (timeout) clearTimeout(timeout);
      if (cancelled) {
        const error = /** @type {Error & {cancelled?: boolean, result?: object}} */ (
      ...
      if (status === 0) {
        ...
        const error = /** @type {Error & {result?: object}} */ (
