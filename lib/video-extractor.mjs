...
export async function extractMkvTracks({ input, tracks = [], outDir }) {
  ...
  // The plan's outputs are predictions; report only files that exist —
  // returning predicted paths would report success for files never written.
  // Verified against a real VobSub MKV (mkvextract v100): a spec ending
  // `.sub` writes exactly `<name>.sub` + `<name>.idx`, matching the
  // prediction above, so a stat failure here means something genuinely went
  // wrong rather than a naming mismatch.
